...
complexity: complex
---
Region #11 of `feat-full-italy-coverage.md`; the parent plan holds the decisions.
...
Facts: slug `friuli-venezia-giulia`, API id `friuli_venezia_giulia`, ISTAT COD_REG 6, names it "Friuli-Venezia Giulia" / en "Friuli-Venezia Giulia".
Karst on the coast, Carnic and Julian Alps inland.
Forest map candidate, to verify: the regional Carta dei tipi forestali.
...
- [ ] Config: `api/src/api/config/regions/friuli_venezia_giulia.yaml` (bbox from the ISTAT boundary, COD_REG 6, forest source and class mapping, iNaturalist place id resolved by name, `model:` overrides only if the gauge check asks); `checks lattice` run for the region, national lapse rates kept unless the fit differs by more than 1 °C/km, either way recorded
- [ ] Species research → `.gavin-root/docs/species-ecology/friuli_venezia_giulia.md` and `api/src/api/config/species/friuli_venezia_giulia/` (start from Tuscany's files): season windows, host trees and habitat affinities, altitude bands, weather rules where regional literature exists; every rule cited with a confidence; regional references added to the shared bibliography; groups the region lacks dropped and said so; `sanity.yaml` with press or blog contrasts for this region's areas and years
- [ ] Data: `api.regions.onboard friuli_venezia_giulia` through history build; forest area within ±10 % of INFC 2015 or explained; counts recorded in `.gavin-root/docs/regions/friuli_venezia_giulia.md`
