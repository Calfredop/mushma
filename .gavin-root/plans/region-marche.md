...
complexity: complex
---
Region #5 of `feat-full-italy-coverage.md`; the parent plan holds the decisions.
...
Facts: slug `marche`, API id `marche`, ISTAT COD_REG 11, names it "Marche" / en "Marche".
Sibillini; Tuscany's neighbour across the Apennines.
Forest map candidate, to verify: Inventario e Carta Forestale della Regione Marche (IFRM).
...
- [x] Config: `api/src/api/config/regions/marche.yaml` (bbox from the ISTAT boundary, COD_REG 11, forest source and class mapping, iNaturalist place id resolved by name, `model:` overrides only if the gauge check asks); `checks lattice` run for the region, national lapse rates kept unless the fit differs by more than 1 °C/km, either way recorded
- [x] [Species research](./region-marche-species.md) → `.gavin-root/docs/species-ecology/marche.md` and `api/src/api/config/species/marche/` (start from Tuscany's files): season windows, host trees and habitat affinities, altitude bands, weather rules where regional literature exists; every rule cited with a confidence; regional references added to the shared bibliography; groups the region lacks dropped and said so; `sanity.yaml` with press or blog contrasts for this region's areas and years
- [ ] Data: `api.regions.onboard marche` through history build; forest area within ±10 % of INFC 2015 or explained; counts recorded in `.gavin-root/docs/regions/marche.md`
