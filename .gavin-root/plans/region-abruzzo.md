...
complexity: complex
---
Region #12 of `feat-full-italy-coverage.md`; the parent plan holds the decisions.
...
Facts: slug `abruzzo`, API id `abruzzo`, ISTAT COD_REG 13, names it "Abruzzo" / en "Abruzzo".
Gran Sasso and Majella beech, the highest Apennine woodland.
...
- [ ] Config: `api/src/api/config/regions/abruzzo.yaml` (bbox from the ISTAT boundary, COD_REG 13, forest source and class mapping, iNaturalist place id resolved by name, `model:` overrides only if the gauge check asks); `checks lattice` run for the region, national lapse rates kept unless the fit differs by more than 1 °C/km, either way recorded
- [ ] Species research → `.gavin-root/docs/species-ecology/abruzzo.md` and `api/src/api/config/species/abruzzo/` (start from Tuscany's files): season windows, host trees and habitat affinities, altitude bands, weather rules where regional literature exists; every rule cited with a confidence; regional references added to the shared bibliography; groups the region lacks dropped and said so; `sanity.yaml` with press or blog contrasts for this region's areas and years
- [ ] Data: `api.regions.onboard abruzzo` through history build; forest area within ±10 % of INFC 2015 or explained; counts recorded in `.gavin-root/docs/regions/abruzzo.md`
