...
| `habitat` | affinity 0–1 per habitat key, optional `response` | the host-weighted share: Σ over the cell's habitat fractions of fraction × affinity (`default` for unlisted habitats); with a `response` trapezoid, that share is saturated (e.g. `[0, 0.3, null, null]` reaches full credit once 30 % of the woods are worth a host), so a cell need not be entirely host wood to score 1 |
