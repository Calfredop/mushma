...
def read_group_cover(
    layers: list[GroupLayer], sources: dict[str, Source], raw: Path, crs: str
) -> gpd.GeoDataFrame:
...
def build(region_name: str = "tuscany", root: Path | None = None) -> dict[str, Path]:
    ...
    group_layers = forest_group_layers(region, vocabulary)
    ...
    clc_field = forest_type_column(types_config, clc_download)
    types_cache = source_cache_dir(raw, types_source_id, clc_download, region.id)

    if group_layers is not None:
        step("forest groups (land cover)")
        cover = read_group_cover(group_layers, sources, raw, crs)
        groups = forest.class_fractions(cover, "group", grid)

        step(f"forest types ({types_source_id})")
        clc = read_vector(
            clc_download,
            types_cache,
            bbox_wgs84=bbox,
            where=types_config.get("where"),
        )
        clc["habitat"] = clc[clc_field].astype(str).map(type_classes)
        types = forest.class_fractions(clc[clc["habitat"].notna()].to_crs(crs), "habitat", grid)
        used_sources = [*(layer.source for layer in group_layers), types_source_id]
    else:
